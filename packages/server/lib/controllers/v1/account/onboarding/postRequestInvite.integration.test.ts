...
async function createSuggestedAccountSession() {
    ...
    expect(requester?.email_verification_token).toBeTruthy();

    const verificationRes = await api.fetch(
        verifyEmailRoute as any,
        {
            method: 'POST',
            body: { token: requester!.email_verification_token! }
        } as any
    );
...
describe(`POST ${requestInviteRoute}`, () => {
    ...
    it('emails active administrators once and permanently consumes the request', async () => {
