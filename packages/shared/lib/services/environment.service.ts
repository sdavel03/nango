...
class EnvironmentService {
    ...
    async createEnvironment(
        ...
        try {
            ...
            const environment = await trx.transaction(async (innerTrx): Promise<Result<DBEnvironment, CreateEnvironmentError>> => {
                const [env] = await innerTrx<DBEnvironment>(TABLE)
                    ...
                    .returning('*');
                if (!env) {
                    const error = new CreateEnvironmentError('creation_failed');
                    report(error, { accountId, environmentName: name });
                    return Err(error);
                }
            ...
            await onNewEnvironment(trx, { accountId, environmentId: environment.value.id, isProduction: isProduction.value });
            return environment;
        } catch (err) {
    ...
    async createDefaultEnvironments(trx: Knex, { accountId }: { accountId: number }): Promise<void> {
        for (const environment of defaultEnvironments) {
            const newEnv = (await this.createEnvironment(trx, { accountId, name: environment })).unwrap();
