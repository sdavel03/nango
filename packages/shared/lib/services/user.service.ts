...
class UserService {
    ...
    async getActiveAdministratorsByAccountId(accountId: number): Promise<DBUser[]> {
        return db.knex.select('*').from<DBUser>('_nango_users').where({ account_id: accountId, suspended: false, role: 'administrator' });
    }
